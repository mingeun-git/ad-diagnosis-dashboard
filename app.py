...
무엇을 보여주는 화면인가
------------------------
데이터 스프린트 팀이 만든 **시스템 2개**(① 어뷰징 탐지 · ② 광고 선별·진단)를
하나의 이야기로 잇는다. 왼쪽 내비게이션 순서가 곧 이야기 순서다:

    왜 이 프로젝트인가 → 1차 EDA 신호 3개 → 가설 검증 → 진단·해법 → 시스템 2개 → 마무리
...
팀원 시스템인 **① 어뷰징 탐지는 2026-08-25 에 자료를 받아 Ⅱ 화면을 실제 내용으로 채웠다**
(원본 `어뷰징시스템_재현폴더/` · 정리본 `팀원1_어뷰징탐지시스템_정리.md` · 상수는 `story.ABUSE_*`).

> 2026-08-26: **개인화 추천 시스템은 프로젝트 범위에서 제외**됐다. 관련 화면·상수·문구를 전부 걷어냈고,
> 시스템은 ① 어뷰징 탐지와 ② 광고 선별·진단 **2개**다.
...
if PAGE == "intro":
    krds.page_header(
        ...
        meta=[("관측 기간", story.PERIOD), ("원본 클릭", "16,854,865건"),
              ("만든 시스템", "2개"), ("문서 기준", story.UPDATED)])

    krds.alert("information", "이 대시보드를 읽는 법",
               "왼쪽 내비게이션 <b>순서가 곧 이야기 순서</b>입니다. "
               "Ⅰ(왜) → Ⅱ·Ⅲ(무엇을 만들었나) → Ⅳ(믿어도 되는 근거) 로 읽으면 "
               "처음 보셔도 “왜 시스템이 2개인가”까지 이해하실 수 있습니다. "
               "<b>Ⅲ 광고 선별·진단</b> 이 본인 담당이자 실제 산출물이 연결된 살아 있는 화면이고, "
               "<b>Ⅱ 어뷰징 탐지</b> 는 팀원 1 의 시스템을 수령·검증해 정리한 화면입니다.")
...
# ══════════════════════════════════════════════════════════ Ⅰ-2. 1차 EDA
    ...
    krds.page_header(
        ...
        desc="데이터·유저·광고 각각을 따로 열어봤다. "
             "<b>세 곳 모두에서 하나씩 신호가 나왔고</b>, 그 신호가 시스템 2개로 이어진다.",
...
# ══════════════════════════════════════════════════════════ Ⅰ-3. 가설 검증
    ...
    krds.cards([
        {"label": "검정한 가설", "value": str(H["n"]), "unit": "개", "tone": "primary",
         "sub": "프로젝트 초기 14개 (요약리포트 2026-07-31 기준)"},
        ...
        {"label": "시스템 결정에 직결", "value": str(H["shown"]), "unit": "개", "tone": "information",
         "sub": "A군 2 · C군 3 — 이 화면에 정리"},
    ])
...
# ══════════════════════════════════════════════════════════ Ⅰ-4. 진단 → 시스템 2개
    ...
    krds.page_header(
        "진단 → 결정 — 문제 2개, 시스템 2개",
        eyebrow="Ⅰ. 왜 이 프로젝트인가",
        desc="검증을 거치자 신호는 두 개의 <b>구조적 문제</b>로 확정됐다 — "
             "<b>측정</b>과 <b>공급</b>. 그리고 둘 다 일회성 분석으로는 풀 수 없다.",
        meta=[("문제", "2개"), ("시스템", "2개"), ("담당", "본인 1 · 팀원 1")])
    ...
    krds.nodes([
        ...
        ("② 광고 선별·진단", "오염 없는 CVR 을 정답 라벨로 학습", "primary"),
    ])
    krds.quote(story.DEPENDENCY_NOTE, "gray")

    krds.section("두 시스템은 서로를 먹여 살린다")
...
# 여기 숫자는 전부 story.ABUSE_* 상수다. 우리가 재계산하지 않는다 (CLAUDE.md D14).
    ...
    # ── ⑫ 시스템 간 연결
    krds.section("이 시스템이 ② 광고 선별에게 주는 것")
    krds.rows([
...
# ══════════════════════════════════════════════════════════ Ⅲ-3. 성과 스코어보드
    ...
    krds.page_header(
        "태그 성과 스코어보드", eyebrow="Ⅲ. 시스템 ② · 산출물 A",
        desc="15개 태그 축 중 하나를 골라, 그 축의 <b>값별 실제 성과</b>를 비교한다. "
             "죽어 있던 기존 분류축(카테고리 88.9%가 한 값)을 대신하는 살아 있는 축이다.",
...
# ══════════════════════════════════════════════════════════ Ⅳ. 마무리
    ...
    krds.page_header(
        ...
        meta=[("시스템", "2개"), ("문서 기준", story.UPDATED)])
